...
test('trusted link completes pairing and persists only verified keys',async({page})=>{
 await page.goto('/');
 ...
 await page.route('https://workspace.test/**',async route=>{
   ...
   if(url.pathname==='/auth/session')return route.fulfill({json:{csrf:'test',workspace:{user:{id:10},threads:[],messages:[],collector_seen:null}}});
 ...
 await page.goto('https://workspace.test/'+fixture.fragment);
 await page.getByRole('button',{name:'Привязать',exact:true}).click();
