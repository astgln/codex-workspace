...
export type WorkspaceState = {encryption?:{v:number;workspace:string};encryption_locked?:boolean;projects?:{id:string;title:string}[];weekly_quota?:{used_percent:number;resets_at:number;observed_at:number}|null;user:{id:number};threads:Thread[];messages:Message[];catalog_updated:number|null;collector_seen:number|null};
