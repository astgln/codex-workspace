...
export type WorkspaceState = {encryption?:{v:number;workspace:string};encryption_locked?:boolean;projects?:{id:string;title:string}[];weekly_quota?:{used_percent:number;resets_at:number;observed_at:number}|null;user:{id:number;role:'owner'|'member';requires_approval?:boolean};threads:Thread[];messages:Message[];members?:{id:number;username:string;projects?:string[];denied_threads?:string[];requires_approval?:boolean;threads:string[]}[];catalog_updated:number|null;collector_seen:number|null};
