...
export async function signOut(){await request('/auth/logout',{});csrfToken='';(await import('../../features/encryption/client')).clearEncryptedSession();}
export async function restoreSession(){const value=await request<{csrf:string;workspace:WorkspaceState}>('/auth/session');csrfToken=value.csrf;return (await import('../../features/encryption/client')).initializeEncryption(value.workspace);}
export class ApiError extends Error { constructor(public status:number,message:string){super(message);} }
export async function request<T>(path:string,body?:unknown):Promise<T>{
  if(!path.startsWith('/web/e2ee/')&&!['/auth/session','/auth/logout','/auth/device/challenge','/auth/device/session','/auth/pairing/offer','/auth/pairing/read'].includes(path))throw new Error('Передача открытого текста отключена для этого пространства.');
