...
export async function request<T>(path:string,body?:unknown):Promise<T>{
  if(!path.startsWith('/web/e2ee/')&&!['/auth/session','/auth/logout','/web/login/config','/web/login/session'].includes(path))throw new Error('Передача открытого текста отключена для этого пространства.');
