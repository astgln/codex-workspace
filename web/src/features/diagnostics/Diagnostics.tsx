...
type Status = {
  ...
  notifications:{devices:number;retrying:number;uncertain:number}|null;
};
...
export function Diagnostics(){
  ...
  return <details className="push-settings small" onToggle={event=>{if(event.currentTarget.open&&!status&&!busy)void refresh();}}>
    ...
    {status&&<>
      ...
      <p>На одобрение: {status.queue.awaiting_approval??0} · В очереди: {status.queue.approved??status.queue.queued??0}<br/>Готовых ответов: {status.completed}</p>
      {status.worker&&<div>
      ...
      {status.notifications&&<><p>Устройств с push: {status.notifications.devices}<br/>Повторных попыток в ожидании: {status.notifications.retrying}<br/>Неизвестный результат: {status.notifications.uncertain}</p>
