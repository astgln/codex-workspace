...
class SealedChannel:
    ...
    def publish(self, scope: str, kind: str, record: str, revision: int, payload: dict):
        if kind not in ('history', 'response', 'catalog', 'push') or not isinstance(payload, dict):
        ...
        return self.api.call('/v2/e2ee/publish', {'envelope': envelope})

    def requests(self, scope: str, after: int):
        ...
        result = self.api.call('/v2/e2ee/read', {'workspace': self.vault.workspace,
                              'scope': scope, 'kind': 'request', 'after': after})
        ...
        for entry in result['records']:
            ...
            if (not isinstance(context, list) or len(context) != 5 or
                    context[:3] != [self.vault.workspace, scope, 'request'] or type(context[4]) is not int or context[4] != 1):
