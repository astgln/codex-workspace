...
from codex_workspace.agent.history_watch import sync_once
...
class WatchTests(unittest.TestCase):
 def test_checkpoint_and_pending_idle_refresh(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   with patch('codex_workspace.agent.history_sync.locate',return_value=path):
    sync_once(api,catalog,'project',Path(tmp),cache)
    with patch('codex_workspace.agent.history_sync.read_public',side_effect=AssertionError('must not rescan idle file')):
     sync_once(api,catalog,'project',Path(tmp),cache)
 ...
 def test_reader_version_change_replays_without_advancing_on_failure(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   with patch('codex_workspace.agent.history_sync.locate',return_value=path), patch('codex_workspace.agent.history_sync.publish',side_effect=OSError('offline')):
    failures={}
   ...
   from codex_workspace.codex.rollout_history import read_public, READER_VERSION
   with patch('codex_workspace.agent.history_sync.locate',return_value=path), patch('codex_workspace.agent.history_sync.read_public',wraps=read_public) as reader:
    sync_once(API(),catalog,'project',Path(tmp),cache)
 ...
 def test_missing_journal_does_not_starve_other_tasks(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   cache={};failures={}
   with patch('codex_workspace.agent.history_sync.locate',side_effect=lambda root,ident:root/ident):
 ...
 def test_quota_retry_keeps_committed_history_cursor(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   with patch('codex_workspace.agent.history_sync.locate',return_value=path),patch('codex_workspace.agent.history_sync.read_public',return_value=read),patch('codex_workspace.agent.history_sync.publish') as publish:
    sync_once(api,catalog,'project',Path(tmp),cache,failures)
    self.assertEqual(failures,{'task':'quota_unavailable'})
    self.assertEqual(cache['task']['offset'],path.stat().st_size)
    self.assertIn('pending_quota',cache['task'])
    api.failing=False
    with patch('codex_workspace.agent.history_sync.read_public',side_effect=AssertionError('no replay')):
     sync_once(api,catalog,'project',Path(tmp),cache,{})
    self.assertNotIn('pending_quota',cache['task'])
    self.assertEqual(publish.call_count,1)

 def test_manual_sync_validates_full_scope_before_any_publication(self):
  from codex_workspace.agent.history_client import sync_catalog
  from codex_workspace.agent.runtime_support import BridgeError
 ...
 def test_manual_sync_preserves_good_tasks_after_missing_journal(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   ...
   catalog={'project_id':'project','threads':[{'id':name,'project_id':'project'} for name in ('missing','good')]}
   with patch('codex_workspace.agent.history_sync.locate',side_effect=lambda root,ident:root/ident),patch('codex_workspace.agent.history_sync.publish') as publish:
