...
from codex_workspace.agent.workspace_client import API, MAX_RESPONSE_BYTES
...
class OutboundTests(unittest.TestCase):
    ...
    def test_api_rejects_host_injection_and_oversized_or_nonobject_replies(self):
        ...
        for path in ('@evil.test/x','//evil.test/x','/v2/x?token=leak','/v2/\\evil'):
            with patch('codex_workspace.agent.workspace_client.urllib.request.build_opener') as opener:
        ...
        for raw in (b'x'*(MAX_RESPONSE_BYTES+1),b'[]'):
            ...
            with patch('codex_workspace.agent.workspace_client.urllib.request.build_opener',return_value=opener):
