...
from codex_workspace.relay import files
...
class FileTests(unittest.TestCase):
    ...
    def upload(self):
        upload=files.handle(self.store,10,'owner','start',self.body)
        files.handle(self.store,10,'owner','chunk',{'id':upload['id'],'index':0,'data':base64.b64encode(self.data).decode()})
        files.handle(self.store,10,'owner','finish',{'id':upload['id']})
        return upload

    def test_immutable_file_bound_to_delivered_request(self):
        upload=self.upload()
        with self.assertRaises(workspace.Forbidden):files.handle(self.store,20,'owner','get',{'id':upload['id'],'index':0})
        with self.assertRaises(workspace.Forbidden):files.handle(self.store,None,'owner','get',{'id':upload['id'],'index':0},True)
        item=self.store.mutate(lambda s:workspace.submit(s,10,'owner',{'thread':'thread-launcher','text':'','attachments':[upload['id']],'request_id':'message-unique-key'},self.now))
        ...
        with self.assertRaises(domain.Rejected):files.handle(self.store,10,'owner','chunk',{'id':upload['id'],'index':0,'data':base64.b64encode(self.data).decode()})
    ...
    def test_read_only_thread_rejects_upload_creation(self):
        self.store.mutate(lambda s:s['catalog']['thread-launcher'].update(read_only=True))
        with self.assertRaises(workspace.Forbidden):files.handle(self.store,10,'owner','start',self.body)

    def test_incomplete_and_wrong_digest_fail(self):
        upload=files.handle(self.store,10,'owner','start',{**self.body,'sha256':'0'*64})
        with self.assertRaises(domain.Rejected):files.handle(self.store,10,'owner','finish',{'id':upload['id']})
        files.handle(self.store,10,'owner','chunk',{'id':upload['id'],'index':0,'data':base64.b64encode(self.data).decode()})
        with self.assertRaises(domain.Rejected):files.handle(self.store,10,'owner','finish',{'id':upload['id']})

    def test_path_and_size_validation(self):
        for change in ({'name':'../file'},{'name':'a\\b'},{'size':files.MAX_FILE+1},{'size':0}):
            with self.assertRaises(domain.Rejected):files.handle(self.store,10,'owner','start',{**self.body,**change})
        with self.assertRaises(domain.Rejected):files.handle(self.store,10,'owner','get',{'id':'../file','index':0})
