...
from codex_workspace.codex.desktop_catalog import discover, refresh
...
class DesktopCatalogTests(unittest.TestCase):
    ...
    def test_activity_requires_live_writer(self):
        ...
        with patch('codex_workspace.codex.desktop_catalog.writer_busy',return_value=False):
            self.assertEqual(discover(self.home,self.previous,cache)['threads'][0]['status'],'unknown')
        with patch('codex_workspace.codex.desktop_catalog.writer_busy',return_value=True):
